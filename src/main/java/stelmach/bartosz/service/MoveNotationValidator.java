...
public class MoveNotationValidator {
    ...
    public boolean isLengthCorrect(String move, boolean isTaking, boolean isPromoting, boolean isChecking, boolean isMating) {
        System.out.println("isLengthCorrect move: " + move);
        System.out.println("move.length(): " + move.length());

        int correctLength = 2;
        if (isTaking) correctLength += 2;
        if (isPromoting) correctLength += 2;
        if (isChecking || isMating) correctLength += 1;
